...
func NewWire(viperViper *viper.Viper, logger *log.Logger) (*app.App, func(), error) {
	...
	inveRepo := repository.NewInvitationsRepository(repositoryRepository)
	teamRepo := repository.NewTeamcountsRepository(repositoryRepository)
	trsfRepo := repository.NewTransferRepository(repositoryRepository)
	nodesRepo := repository.NewNodesRepository(repositoryRepository)
	userService := service.NewUserService(serviceService,viperViper, userRepository,inveRepo,teamRepo,trsfRepo,nodesRepo)
	userHandler := handler.NewUserHandler(handlerHandler, userService)
	httpServer := server.NewHTTPServer(logger, viperViper, jwtJWT, userHandler)
	job := server.NewJob(logger,userService)
